...
describe('/test/cdata.test.ts', () => {
  ...
  it('complex', async () => {
    ...
    assert(result.rss.channel.item.description._value === '在过去的 10 年里，iOS 一点点成长为了苹果最重要的现金牛');
    assert(result.rss.channel.item.link._value === 'https://www.zhihu.com/question/294345168');
  });
});
